...
> 登入方式：mineflayer 內建 Microsoft 裝置代碼登入（終端會顯示驗證網址與代碼）。  

## 1. 專案概觀
- Mineflayer 自動化機器人，支援 Microsoft / 離線登入。
- 主要功能：聊天橋接、TPA 白名單、自動每日領獎、職業選擇、RPG 分流切換、自動轉帳、死亡 / 斷線自動處理與重連、資源包自動接受。
- 設定檔與登入快取放在執行檔或專案同層，避免寫入系統目錄。
...
```jsonc
{
  ...
  "language": "zh-tw",
  "whitelist": ["Konenala21", "hoto_kokoyu"],
...
## 4. 安裝與開發模式執行
```powershell
cd /mnt/e/bot編寫/基礎bot_雲界   # 或對應 Windows 路徑
npm install
npm start
```
首次登入會顯示：
...
- `job`：開啟職業菜單並選擇預設職業。
- `gorpg`：切換到 RPG 分流。
> 只有在 `whitelist` 內的玩家私訊才會觸發；請使用 `/msg <BotName> <指令>`（不要用 `/m`）。
...
## 9. 開發提示
- PowerShell 指令相容；打包目標 `node18-win-x64`。
- 重要變數與函式皆附中文註解，請參考 `src/main.js`。 
