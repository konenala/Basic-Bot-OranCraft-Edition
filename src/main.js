...
// Priority: 1. CLI argument, 2. Environment variable, 3. Default file next to executable (pkg)
// or in the working/project directory (development).

// 函式：解析預設設定檔路徑（依執行環境挑選候選）
function resolveDefaultConfigPath() {
  const candidates = []

  // pkg 執行檔：設定檔應放在可寫的執行檔同層
  if (process.pkg) {
    candidates.push(path.join(executableDir, 'config.json'))
  } else {
    // 開發模式：優先使用目前工作目錄，其次為專案根目錄，再退回到執行檔目錄
...
async function startBot() {
  ...
  bot.once('spawn', () => {
    ...
    // 伺服器新版訊息格式（系統提示）
    bot.chatAddPattern(/^\[系統\]\s*(.+?)\s*希望傳送到您這裡 \(\[✔\]\s*\/\s*\[✘\]\)$/, 'tpa_to_me', 'TPA請求')
    bot.chatAddPattern(/^\[系統\]\s*(.+?)\s*希望您傳送到他那裡 \(\[✔\]\s*\/\s*\[✘\]\)$/, 'tpa_from_me', 'TPA請求')
